...
import { CheckCircle2, Clock3, Copy, Send, TimerReset } from "lucide-react";
...
export function ContactHero({ name, username, headline, statement, status, style }: { name: string; username: string; headline: string; statement: string; status: string[]; style: string }) {
  return (
    <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mt-5 rounded-3xl border border-white/70 bg-white/75 p-6 text-center shadow-[0_28px_56px_-32px_rgba(79,70,229,0.55)] backdrop-blur md:p-8">
      <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-3xl bg-gradient-to-br from-violet-500 to-indigo-600 text-2xl font-bold text-white shadow-xl">RH</div>
      ...
      <p className="mt-4 text-sm text-zinc-600">{style}</p>
    </motion.section>
  );
}
...
export function ContactMethodsGrid({ items }: { items: ContactMethod[] }) {
  return (
    <section className="mt-6">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {items.map((item) => {
          ...
          return (
            <article key={item.platform} className="rounded-2xl border border-violet-100 bg-white p-5 shadow-[0_20px_46px_-30px_rgba(79,70,229,0.45)] transition hover:-translate-y-0.5">
...
export const PersonalMessage = () => <section className="mt-6 rounded-3xl border border-violet-200 bg-gradient-to-br from-white to-violet-50/70 p-6"><p className="text-sm leading-relaxed text-zinc-700">If you’re building something meaningful, exploring interesting ideas, or simply want to connect thoughtfully, feel free to reach out.</p></section>;
