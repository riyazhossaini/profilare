...
export function SkillCTA({ username }: { username: string }) {
  return (
    <motion.section
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
        ...
        <Link to={`/profile/${username}/identity`} className="rounded-full bg-gradient-to-r from-violet-500 to-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md transition hover:brightness-110">View Identity</Link>
        ...
        <Link to={`/profile/${username}/skills`} className="rounded-full border border-violet-200 bg-white px-5 py-2.5 text-sm font-semibold text-violet-700 transition hover:bg-violet-50">Explore Learning Journey</Link>
      </div>
    </motion.section>
  );
}
