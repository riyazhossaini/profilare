...
export function LinksPage() {
  ...
  const [startupItems, setStartupItems] = useState<StartupLink[]>(() =>
    load(
      STARTUP_KEY,
      startupLinks.map((i) => ({ name: i.name, tagline: i.tagline, stage: i.stage, category: i.category, url: i.url })),
    ),
  );
        ...
        <EditableSectionBlock sectionId="links-grid" label="Main Links">
          <LinksGrid
            ...
            onEdit={(filteredIndex) => {
              ...
              const original = mainItems.findIndex(
                (item) => item.name === target.name && item.handle === target.handle && item.url === target.url,
              );
              if (original >= 0) setEditingMain(original);
            }}
          />
        </EditableSectionBlock>

        <EditableSectionBlock sectionId="links-startup" label="Startup Links">
          <StartupLinksSection items={startupItems.map((item) => ({ ...item }))} onEdit={(index) => setEditingStartup(index)} />
        </EditableSectionBlock>
